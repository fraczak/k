...
- Variant values are written using "|" (pipe) followed by the variant tag.
...
- Angle brackets are for type definitions and merge expressions; use "`| tag`" for a variant construction.
...
Filters and type expressions act as a type annotation, i.e., an identity
...
The target type `${ byte byte, bit overflown }` corresponds to pattern `?{ $byte byte, $bit overflown }`.
...
This could be a generic and compact way to encode and decode typed values.
