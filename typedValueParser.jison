...
%token NAME STRING
%token  AT LC RC LB RB COMMA COL
%token EOF
...
str : STRING                            { $$ = getToken(yytext,yy,_$); $$.value = fromEscString($$.value);}
    ...
    ;
type_id: NAME                           { $$ = getToken(yytext,yy,_$); };
...
input_with_eof
    : typed_value EOF   { return $1.value; }
    ;

typed_value
    : at type_id exp   {
                      $$ = {value: new TypedValue("@" + $2.value, $3.value), start: $1.start, end: $3.end};
                    }
    ;
