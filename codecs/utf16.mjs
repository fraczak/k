...
async function main() {
  ...
  if (args.length !== 1 || (args[0] !== "--parse" && args[0] !== "--print")) {
    ...
    console.error("  --parse  read UTF-16 text from stdin (BOM-aware), write binary KPV2 encoding of k string");
    console.error("  --print  read binary KPV2 encoding of k string, write UTF-16LE text with BOM");
    exit(1);
  }
