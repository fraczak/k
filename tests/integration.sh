...
./objects/compile.mjs --format kvm '()' | grep -q '"format": "k-vm"'
./objects/compile.mjs --format kvm '()' > "$TMP_DIR/stdin.kvm"
grep -q '"format": "k-vm"' "$TMP_DIR/stdin.kvm"
grep -q '"layer": "KVM-P"' "$TMP_DIR/stdin.kvm"
grep -q '"isPolymorphic": true' "$TMP_DIR/stdin.kvm"
grep -q '"__main__"' "$TMP_DIR/stdin.kvm"
./objects/compile.mjs "$TMP_DIR/stdin.ko" "$TMP_DIR/poly-object.kvm"
grep -q '"layer": "KVM-P"' "$TMP_DIR/poly-object.kvm"
grep -q '"isPolymorphic": true' "$TMP_DIR/poly-object.kvm"
